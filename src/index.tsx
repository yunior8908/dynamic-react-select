...
export default function CustomSelect<Name extends string>({
  ...
  const handleInputSearchRef = (node: HTMLInputElement) => {
    if (node && isSearchable && open) {
      node.focus()
    }
  }
      ...
      >
        {isSearchable ? (
          <li className={clsx('search-wrapper', classes.input)}>
            ...
            <input ref={handleInputSearchRef} type='text' onChange={(e) => handleInputChange?.(e.target.value)} />
